...
# ── Aqua0 CCTPComposer Redeployment Script ────────────────────────────────
# Redeploys CCTPComposer and swaps it in BridgeRegistry.
#
# Usage:
#   CHAIN=base DEPLOYER_PRIVATE_KEY=$KEY bash script/redeploy-cctp-composer.sh [--dry-run] [--no-verify]
#   CHAIN=unichain DEPLOYER_PRIVATE_KEY=$KEY bash script/redeploy-cctp-composer.sh [--dry-run] [--no-verify]
...
OLD_COMPOSER=$(cd /tmp && python3 -c "import json; print(json.load(open('$DEPLOY_JSON'))['cctpComposer'])")
BRIDGE_REGISTRY=$(cd /tmp && python3 -c "import json; print(json.load(open('$DEPLOY_JSON'))['bridgeRegistry'])")

echo "============================================"
echo "  Aqua0 CCTPComposer Redeployment"
echo "============================================"
echo "  Chain:            $CHAIN"
echo "  Deployer:         $DEPLOYER_ADDRESS"
echo "  Old CCTPComposer: $OLD_COMPOSER"
...
FORGE_CMD=(
    forge script script/RedeployCCTPComposer.s.sol:RedeployCCTPComposer
    --rpc-url "$RPC_URL"
    --slow
    -vvvv
)
...
if [[ -f "$DEPLOY_JSON" ]]; then
    NEW_COMPOSER=$(cd /tmp && python3 -c "import json; print(json.load(open('$DEPLOY_JSON'))['cctpComposer'])")

    echo ""
    echo "============================================"
    echo "  CCTPComposer Redeployed ($CHAIN)"
    echo "============================================"
