...
# ── Aqua0 StargateAdapter Redeployment Script ────────────────────────────
# Redeploys StargateAdapter (native ETH pool fix) and swaps it in BridgeRegistry.
#
# Usage:
#   CHAIN=base DEPLOYER_PRIVATE_KEY=$KEY bash script/redeploy-stargate-adapter.sh [--dry-run] [--no-verify]
#   CHAIN=unichain DEPLOYER_PRIVATE_KEY=$KEY bash script/redeploy-stargate-adapter.sh [--dry-run] [--no-verify]
...
OLD_ADAPTER=$(cd /tmp && python3 -c "import json; print(json.load(open('$DEPLOY_JSON'))['stargateAdapter'])")
BRIDGE_REGISTRY=$(cd /tmp && python3 -c "import json; print(json.load(open('$DEPLOY_JSON'))['bridgeRegistry'])")

echo "============================================"
echo "  Aqua0 StargateAdapter Redeployment"
echo "============================================"
echo "  Chain:               $CHAIN"
echo "  Deployer:            $DEPLOYER_ADDRESS"
echo "  Old StargateAdapter: $OLD_ADAPTER"
...
FORGE_CMD=(
    forge script script/RedeployStargateAdapter.s.sol:RedeployStargateAdapter
    --rpc-url "$RPC_URL"
    --slow
    -vvvv
)
...
if [[ -f "$DEPLOY_JSON" ]]; then
    NEW_ADAPTER=$(cd /tmp && python3 -c "import json; print(json.load(open('$DEPLOY_JSON'))['stargateAdapter'])")

    echo ""
    echo "============================================"
    echo "  StargateAdapter Redeployed ($CHAIN)"
    echo "============================================"
    echo "  Old: $OLD_ADAPTER"
    echo "  New: $NEW_ADAPTER"
    echo ""
